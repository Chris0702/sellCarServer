exports.on = function(app) {
    ...
    app.post(preRestApi + '/saveCarInfo', function(req, res) {
        ...
        car.saveCarInfo(car_id, company, name,color, price, special_price,description, (result, msg) => {
            if (!result) {
    ...
    app.get(preRestApi + '/getCarsInfoById', function(req, res) {
        let response = {
            'resStatus': 0,
            'resString': ''
        }
        let id = req.query.id;
        car.getCarsInfoById(id, (result, msg) => {
            if (!result) {
                response.resStatus = 1;
            }
            response.resString = msg
